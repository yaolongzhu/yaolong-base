...
public class ModelRecordElResolver extends ELResolver {
	...
	/**
	 * 设置为 true 时，使用生成器生成的实现了 IBean 接口的 Class 将被当成 Model 来处理， getter 不被 jsp/jstl
	 * 用来输出数据，仍然使用 model.get(String attr) 来输出数据。
	 * 
	 * 有利于在关联查询时输出无 getter 方法的字段值。建议mysql数据表中的字段采用驼峰命名， 表名仍然用下划线方式命名。
	 * resolveBeanAsModel 默认值为 false。
	 * 
	...
	/**
	 * Compatible for JspRender.setSupportActiveRecord(true); Delete it in the
	 * future
	 */
