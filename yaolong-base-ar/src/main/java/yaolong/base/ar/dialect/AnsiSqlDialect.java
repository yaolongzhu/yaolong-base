...
import yaolong.base.ar.CPI;
import yaolong.base.ar.Model;
import yaolong.base.ar.ModelBuilder;
...
public class AnsiSqlDialect extends Dialect {
	...
	public Page<Record> takeOverDbPaginate(Connection conn, int pageNumber, int pageSize, Boolean isGroupBySql,
		...
		PreparedStatement pst = conn.prepareStatement(sql.toString(), ResultSet.TYPE_FORWARD_ONLY,
	...
	public Page<? extends Model> takeOverModelPaginate(Connection conn, Class<? extends Model> modelClass,
			int pageNumber, int pageSize, Boolean isGroupBySql, String select, String sqlExceptSelect, Object... paras)
			throws Exception {
		String totalRowSql = "select count(*) " + replaceOrderBy(sqlExceptSelect);
		List result = CPI.query(conn, totalRowSql, paras);
		int size = result.size();
