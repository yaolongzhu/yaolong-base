...
public class DbBase {
	...
	protected static <T> List<T> queryForCount(Config config, Connection conn, String select, String sql,
			Object... paras) throws SQLException {
		return dbPro.queryForCount(config, conn, select, sql, paras);
	}
	...
	public static Page<Record> paginateByCache(String cacheName, Object key, int pageNumber, int pageSize,
		...
		return dbPro.paginateByCache(cacheName, key, pageNumber, pageSize, isGroupBySql, select, sqlExceptSelect,
				paras);
	}
	...
	public static void setDbPro(DbPro dbPro) {
		DbBase.dbPro = dbPro;
	}
